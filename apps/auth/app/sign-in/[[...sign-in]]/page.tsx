  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        console.log("Sign-in needs further steps:", result);
        setError("Yêu cầu thêm bước xác thực. Tính năng đang cập nhật.");
      }
    } catch (err: any) {
      console.error(err);
      setError(err.errors?.[0]?.message || "Đã xảy ra lỗi khi đăng nhập.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="w-full max-w-md bg-background/70 backdrop-blur-xl shadow-2xl border border-border/60 rounded-2xl p-8">
          <div className="mb-8 flex flex-col items-center text-center">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-foreground sm:text-3xl mb-2">
              Đăng nhập <span className="bg-gradient-to-br from-primary via-rose-500 to-orange-500 bg-clip-text text-transparent drop-shadow-sm">povixa</span>
            </h1>
